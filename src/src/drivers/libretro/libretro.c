...
void get_mouse_input(unsigned port, uint32_t *zapdata)
{
   ...
   if (zappermode == RetroMouse) /* mouse device */
   {
      ...
      mzx = input_cb( port, RETRO_DEVICE_LIGHTGUN, 0, RETRO_DEVICE_ID_LIGHTGUN_SCREEN_X );

      mzy = input_cb( port, RETRO_DEVICE_LIGHTGUN, 0, RETRO_DEVICE_ID_LIGHTGUN_SCREEN_Y );

      zapdata[0] = (mzx + (0x7FFF + offset_x)) * max_width  / ((0x7FFF + offset_x) * 2);
      zapdata[1] = (mzy + (0x7FFF + offset_y)) * max_height  / ((0x7FFF + offset_y) * 2);
